...
BYTE GetUserPDPType(ULONG *pPDPType)
{
    CHAR selOption[OPTION_LEN];
    CHAR *pEndOfLine = NULL;
    BYTE pdpType = 0;
    ...
    {
        /* Print the menu */
        fprintf( stderr, "\nPlease input the PDP type (0-IPV4, 1-PPP, 2-IPV6 and 3-IPV4V6, or press <Enter> to exit: ");
...
void ModifyProfileSettings()
{
    ...
    while( 1 )
    {
       ...
       while(1)
       {
        ...
        returnCode = GetUserPDPType(&profileInfo.PDPType);
        if( ENTER_KEY != returnCode )
        {
            pdpType = &profileInfo.PDPType;
        }
        returnCode = 0;
...
static void DisplayAllProfiles()
{
    ...
    for( profileId = MIN_PROFILES; profileId <= MAX_PROFILES; profileId++ )
    {
        ...
        memset(PDPString, 0, sizeof(PDPString));
        switch(PDPType)
        {
            case 0:
            default:
                memcpy(PDPString, "IPV4", 4);
                break;
            case 1:
                memcpy(PDPString, "PPP", 4);
                break;
            case 2:
                memcpy(PDPString, "IPV6", 4);
                break;
            case 3:
                memcpy(PDPString, "IPV4V6", 6);
                break;
        }

        /* Display the retrieved profile information */
        fprintf( stderr, "%*d%*s%*s%*s%*s%*u%*s%*s%*s\n",
                         w3, profileId, w8, PDPString, w20, bufIPAddress,
                         w20, bufPrimaryDNS, w20, bufSecondaryDNS, w5, authentication,
                         w20, profileName, w20, APNName, w20, Username );
    ...
    nasGetSigInfoResp       *pSigInfo )
{
    CHAR respBuf[MAX_FIELD_SIZE];
     FILE    *my_pFile;
     my_pFile = fopen( "MyConnectionManager.txt", "w" );
    /* Reset the RSS values on user window */
    if( display == FALSE )
    {
        UpdateUserDisplay( eECIO, "UNKOWN" );
...
static void DisplayDeviceinfo()
{
    ...
    UIMReadTransReq.fileIndex   = (fileInfo){ 0x6F07, 0x04, {0x3F00,0x7F20} };
    ...
    /* Get the IMSI */
    resultCode = SLQSUIMReadTransparent( &UIMReadTransReq, &UIMReadTransResp );
    if( eQCWWAN_ERR_NONE != resultCode )
    {
        UIMReadTransReq.fileIndex = (fileInfo){ 0x6F07, 0x04, {0x3F00,0x7FFF} };
        resultCode = SLQSUIMReadTransparent( &UIMReadTransReq, &UIMReadTransResp );
    }
    lcount=0;

    if ( NULL != UIMReadTransResp.pReadResult && UIMReadTransReq.fileIndex.fileID == 0x6F07)
    {
        for(count=1; count <UIMReadTransResp.pReadResult->contentLen;count++)
        {
            if(count!=1)/*Skip the first nibble */
            {
                responseBuffer[lcount++]= (UIMReadTransResp.pReadResult->content[count] & 0x0f) + 0x30;
            }
            responseBuffer[lcount++]= ((UIMReadTransResp.pReadResult->content[count]>>4) & 0x0f) + 0x30;
        }
        responseBuffer[lcount] = '\0';
    }
